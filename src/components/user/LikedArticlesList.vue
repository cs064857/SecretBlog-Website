<template>
    <div class="user-summary-main-content-likes">
        <el-scrollbar distance="50" class="likes-scrollbar" @end-reached="loadMore">
...
// 獲取喜歡文章的 API
const fetchLikedArticles = async (page: number) => {
    const userId = route.params.userId;
    if (!userId) return null;

    try {
        const { data } = await http({
            url: http.adornUrl(`/ams/articles/users/${userId}/liked-articles`),
            method: 'get',
        ...
        if (data.code === '200' && data.data) {
            if (Array.isArray(data.data)) {
                return data.data.filter(article => article.isLiked === 1);
            } else {
                const pageData = data.data as PageData<ArtActionInter>;
                return {
                    records: pageData.records.filter(article => article.isLiked === 1),
                    current: pageData.current,
                    pages: pageData.pages
                };
            }
        } else {
            ElMessage.error(data.msg || '獲取喜歡文章失敗');
            return null;
        }
    } catch (error) {
        console.error('獲取喜歡文章失敗:', error);
        ElMessage.error('獲取喜歡文章失敗，請稍後再試');
        return null;
    }
};
...
.liked {
    ...
    padding: 2px 6px;
    border-radius: 4px;
    background-color: var(--bg-rgba-255-99-71-0p3);
}
</style>
